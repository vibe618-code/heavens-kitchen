...
const AdminDashboard = ({
  ...
  const saveDishEdit = () => {
    if (editingDish) setDishes(dishes.map((d) => (d.id === editingDish.id ? editingDish : d)));
    setEditingDish(null);
  };
  const deleteDish = (id: number) => setDishes(dishes.filter((d) => d.id !== id));
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button
                onClick={() => {
                  ...
                  if (code && !isNaN(pct)) setDiscountCodes({ ...discountCodes, [code]: pct });
                }}
                className="px-4 py-2 bg-gold text-obsidian rounded-xl font-semibold"
              >
                Add
